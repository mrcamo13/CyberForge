"""case_runner.py — FORENSICS CyberForge
...
def _registry_path() -> str:
    here = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(here, "..", "content", "registry.json")
...
def load_registry() -> dict:
    """Load registry, normalising 'challenges' -> 'cases' if needed."""
    path = _registry_path()
...
def display_intro(case_data: dict, tool_cmds: list) -> None:
    ...
    clear_screen()
    case_num = case_data.get("id", "").upper()
    title = case_data.get("title", "").upper()
    print_header(f"[ {case_num} -- {title} ]")
    print_divider()
...
def _print_help(tool_cmds: list, multi_step: bool = False) -> None:
    ...
    if aliases:
        print_info(f"  {primary:<10}  -- run the forensic tool  (also: {aliases}, tools)")
    else:
        print_info(f"  {primary:<10}  -- run the forensic tool  (also: tools)")
...
def _show_step_prompt(step_idx: int, n_steps: int, step: dict) -> None:
    """Print the step question header."""
    print_divider()
...
def _find_nudge(cmd: str, wrong_answer_hints: list) -> str:
    """Return a contextual nudge if the wrong answer matches a pattern."""
...
# ---------------------------------------------------------------------------
# Shared command handlers (used by both loops)
# ---------------------------------------------------------------------------
...
def _run_step_loop(
    ...
    start_step = save_data["metrics"][case_id].get("current_step", 0)
    # If they completed but are replaying, restart from 0
    if is_replay:
        start_step = 0

    for step_idx in range(start_step, n_steps):
        step = steps[step_idx]
        step_hints_used = 0  # hints used on THIS step in this session
        ...
        while True:
            ...
            if cmd == "hint":
                ...
                if step_hints_used < len(step_hints):
                    ...
                    step_hints_used += 1
                    # Accumulate into case-level total
                    ...
                    print_warning("No more hints for this step. Try the forensic tool.")
                continue
            ...
            if cmd == "quit":
                ...
                continue  # only reaches here if user said 'n'
            ...
            if cmd in valid:
                ...
                # Advance saved step position
                save_data["metrics"][case_id]["current_step"] = step_idx + 1
                write_save(save_data)

                if step_idx + 1 < n_steps:
                    # More steps to go — show progress and continue
                    print_divider()
                    if not is_replay:
                    ...
                    print()
                    break  # break inner while → for loop advances step_idx
                else:
                    # Final step — fall through to completion
                    break
            else:
...
def _run_single_loop(
    ...
    """Original single-question command loop. Used when no 'steps' key present."""
    print_divider()
    print_warning(f"  {case_data['challenge']}")
    print_divider()
    ...
    while True:
        ...
        if cmd == "hint":
            ...
            if hints_used < 4:
                ...
                print_muted(
                    f"  Hint {hints_used}/4 | "
                    f"XP if correct now: {xp_now} | "
                    f"Remaining: {4 - hints_used}"
                )
            else:
                print_warning("No more hints. Try the forensic tool.")
            continue
...
def _handle_completion(
    ...
    if n_steps > 1:
        print_header(f"CASE CLOSED  --  ALL {n_steps} STEPS COMPLETE")
    else:
        print_header("CASE CLOSED")
    print_divider()
