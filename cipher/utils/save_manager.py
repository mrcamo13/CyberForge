"""save_manager.py — CIPHER CyberForge
...
def _default_save(player_name: str, track: str) -> dict:
    """Return a new save dict with all fields from DATA_MODEL.md §1."""
...
def write_save(save_data: dict) -> None:
    ...
    # Step 2 — verify the .tmp parses
    with open(tmp, "r", encoding="utf-8") as fh:
        json.load(fh)  # raises if corrupt

    # Step 3 — rename .tmp → .json (remove first on Windows)
...
def load_save(player_name: str) -> dict | None:
    """Load and validate save. Returns None if file missing or corrupt."""
    ...
    try:
        ...
        # Rename corrupted file and return None
        if os.path.exists(primary):
    ...
    if not _validate_schema(data):
        # Missing fields — attempt migration instead of wiping
...
def load_with_fallback(player_name: str) -> dict | None:
    """Try primary → try backup → return None (caller creates new save).
    ...
    # Both corrupted
    print("Both save and backup were corrupted. Starting fresh.")
    return None
...
def list_saves() -> list:
    ...
    for filename in os.listdir(saves):
        ...
        try:
            ...
            pass
        result.append({"name": name, "last_played": last_played})
